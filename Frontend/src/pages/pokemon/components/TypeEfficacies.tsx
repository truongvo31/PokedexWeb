...
const PokemonTypeEfficacies = ({ typeEfficacy }: { typeEfficacy: PokemonTypeEfficacyDto[] }) => {
  return (
    typeEfficacy && (
          ...
          <TableHeader>
            <TableRow>
              {columns.map((column) => (
                <TableHeaderCell
                  key={column.key}
                  className={column.className + ' *:justify-center'}
                >
                  {column.label}
                </TableHeaderCell>
              ))}
...
export default PokemonTypeEfficacies;
